...
function getResponse(tabId: number, requestId: string, cb: (resp: any) => void) {
  chrome.debugger.sendCommand(
    ...
    (response: any) => {
      ...
      chrome.runtime.lastError && console.log(`lasterror thrown for requestId ${requestId}`)
...
chrome.debugger.onEvent.addListener((source, method, params: any) => {
  if (method === 'Network.responseReceived' && source.tabId) {
    ...
    //  getResponse(source.tabId!, params.requestId, (resp) => {
    // Dashboard 抽卡数据
    ...
    // ttps://game.granbluefantasy.jp/item/article_list_by_filter_mode?_=1699265025524&t=1
    // Dashboard 体力道具数据
