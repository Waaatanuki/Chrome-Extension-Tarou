  ...
  <el-card body-style="padding: 10px" header-class="my-card-header" h-full w-300px>
    <template #header>
      <div flex justify-between>
        ...
        <el-tooltip v-if="skipQuest.updateTime" content="Last updated" placement="top">
          {{ formatEventDate(skipQuest.updateTime) }}
        </el-tooltip>
      </div>
    </template>
    <template v-if="skipQuest.list.length">
      <el-alert v-if="skipQuest.list.every(item => item.limitedCount === 0)" title="Attempts reached limit" type="success" :center="true" :closable="false" />
    ...
    <el-alert v-else title="Please load skip quest data first" type="info" :center="true" :closable="false" />
  </el-card>
</template>
